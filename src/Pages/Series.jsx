...
export default function Series() {
  ...
  const filteredSeries = videos.filter(
    (video) =>
      video.type === "Dizi" && video.title.toLowerCase().includes(searchBar.toLowerCase())
  );
        ...
        {searchBar && (
          <div className="searchResults">
            {filteredSeries.length > 0 ? (
              <p>{filteredSeries.length} dizi bulundu</p>
            ) : (
              <p>Üzgünüm aradığınız dizi bulunamadı 😔</p>
            )}
          </div>
        )}
